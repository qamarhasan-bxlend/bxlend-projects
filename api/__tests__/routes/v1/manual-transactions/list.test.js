...
const endpoint = "/v1/manual-transactions";
...
it("should list Manual Transactions", async () => {
  ...
  const manual_withdraw_transaction = await factory.manualWithdrawTransaction(user, wallet, bankAccount);
...
it("should list 0 Manual Transactions, if page doesn't exist", async () => {
  ...
  await factory.manualWithdrawTransaction(user, wallet, bankAccount);
  await factory.manualDepositTransaction(user, wallet, cryptoCurrency, bankAccount);
...
it("should list only limited amount of Manual Transactions", async () => {
  ...
  const manual_transactions = await Promise.all([
    factory.manualWithdrawTransaction(user, wallet, bankAccount),
    factory.manualDepositTransaction(user, wallet, cryptoCurrency, bankAccount),
  ]);
