...
it("should resend user's phone number verification code", async () => {
  ...
  const { body } = await http()
    .post("/v1/users/me/phone-number/verification/resend")
...
it("should resend user's phone number verification code & cancel previous request", async () => {
  const { factory, http } = require("@tests/internals");
  const { STATUS_CODE, STATUS_MESSAGE, AUTH_SCOPE, VERIFICATION_STATUS } = require("@src/constants");
  const { User, Verification } = require("@src/models");
  const { Vonage } = require("@src/lib");
...
it("shouldn't allow resending user's phone number verification code if it's already verified", async () => {
  const { factory, http } = require("@tests/internals");
...
it("shouldn't be able to resend user's phone number verification code without having the required scope(s)", async () => {
  const { factory, http } = require("@tests/internals");
...
it("shouldn't be able to resend another user's phone number verification", async () => {
  const { factory, http } = require("@tests/internals");
  const { STATUS_CODE, AUTH_SCOPE, ERROR } = require("@src/constants");
  const { Verification } = require("@src/models");
  const { Vonage } = require("@src/lib");
  ...
  const { body } = await http()
    .post(`/v1/users/${ anotherUser._id }/phone-number/verification/resend`)
    .set("Authorization", `Bearer ${ accessToken.jti }`)
    .send()
    .expect(STATUS_CODE.FORBIDDEN);
