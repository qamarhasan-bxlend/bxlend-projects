...
it("should update user's email address", async () => {
...
it("should update user's email address & cancel previous request", async () => {
  const { factory, http } = require("@tests/internals");
  const { STATUS_CODE, STATUS_MESSAGE, AUTH_SCOPE, VERIFICATION_STATUS } = require("@src/constants");
  const { User, Verification, EmailVerification } = require("@src/models");
  const { Mailgun } = require("@src/lib");
...
it("should just respond ok if it's the same email address", async () => {
  const { factory, http } = require("@tests/internals");
  ...
  const user = await factory.user({ email, password });
...
it("shouldn't be able to update user's email address without having the required scope(s)", async () => {
  const { factory, http } = require("@tests/internals");
...
it("shouldn't be able to update another user's email address", async () => {
  const { factory, http } = require("@tests/internals");
  ...
  const { body } = await http()
    .patch(`/v1/users/${ anotherUser._id }/email`)
    .set("Authorization", `Bearer ${ accessToken.jti }`)
    .send({
      email,
      password,
