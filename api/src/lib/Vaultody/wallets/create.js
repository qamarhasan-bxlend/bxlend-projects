...
const CONTROLLER = [
  ...
  async function createWithdrawTransactionV1Controller(req, res) {
    ...
    try {
      ...
      if (tokenIdentifier) {
        if (withdrawingBlockchain.protocol_type == PROTOCOL_TYPE.TRC_BASED) {
        ...
        if (withdrawingBlockchain.protocol_type == PROTOCOL_TYPE.TRC_BASED) {
          transactionResponse = await Vaultody.createSingleFeelessTransaction(
            withdrawingBlockchain.vaultody_name,
            network.network_name,
            internalWallet.address,
            updated_quantity,
        ...
        else if (withdrawingBlockchain.protocol_type == PROTOCOL_TYPE.ACCOUNT_BASED) {
        ...
        else if (withdrawingBlockchain.protocol_type == PROTOCOL_TYPE.UTXO_BASED) {
      ...
      console.error("Error in transaction:", error.error.details);
