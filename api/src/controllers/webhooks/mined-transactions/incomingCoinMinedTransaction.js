...
const CONTROLLER = [
  ...
  //   }),
  // }),
  async function incomingCoinMinedTransactionV1Controller(req, res) {
    const { body} = req;
    ...
    else if(req.body.data.event =="INCOMING_CONFIRMED_COIN_TX"){
      console.log(req.body);
      console.log(req.body.data.item.minedInBlock)
    }
    else if(req.body.data.event =="INCOMING_CONFIRMED_TOKEN_TX"){
      console.log(req.body);
      console.log(req.body.data.item.minedInBlock)
    }
    ...
    return res.status(200).send("Done");s
