...
const CONTROLLER = [
  ...
  async function showWalletAddressV1Controller(req, res) {
    const {
      user,
      params: { currency },
      query: { select },
    ...
    let wallet;
    const currency_kind = currency.kind;
    const crypto_type = currency.crypto_type;
    try {
      wallet = await showOrCreateWalletService({ user, currency_code, currency_kind, crypto_type }, null, {
